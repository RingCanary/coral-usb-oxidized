...
RUN_ID="m5-eo-rule-refine-probe-$(date -u +%Y%m%dT%H%M%SZ)"
...
# Only compact fatal windows from the previous targeted refinement.
windows = {
    'f7056': [(338, 340), (5518, 5658)],
    'f8976': [(338, 504), (7042, 7253)],
    'f9872': [(338, 442), (1336, 1346), (1347, 1357), (1359, 1432)],
}
...
for family, fam_windows in windows.items():
    ...
    for widx, (start, end) in enumerate(fam_windows):
        ...
        probes = []
        for ridx, rule in enumerate(window_rules):
            plen, off, val = rule
            kept = [r for r in rules if r != rule]
            name = f'w{widx:02d}_r{ridx:02d}_off{off}'
            lines = [f'# minus single rule {family} {start}..{end} off={off} val=0x{val:02x}']
            for p, o, v in kept:
                lines.append(f'{p} {o} 0x{v:02x}')
            (fam_out / f'eo_minus_{name}.patchspec').write_text('\n'.join(lines) + '\n')
            probes.append({
                'name': name,
                'offset': off,
                'value': val,
    ...
    (fam_out / 'RULE_REFINE_META.json').write_text(json.dumps(meta, indent=2) + '\n')
    with (fam_out / 'RULE_REFINE_META.txt').open('w', encoding='utf-8') as f:
        ...
        for w in meta['windows']:
            ...
            for p in w['probes']:
                f.write(f"  {p['name']}: off={p['offset']} val=0x{p['value']:02x}\n")
PY
...
for family in f7056 f8976 f9872; do
  meta="$OUT_DIR/$family/RULE_REFINE_META.json"
...
for fam_dir in sorted(out_dir.iterdir()):
    if not fam_dir.is_dir():
        continue
    meta = json.loads((fam_dir / 'RULE_REFINE_META.json').read_text())
    ...
    for w in meta['windows']:
        probes = []
        for p in w['probes']:
            c = cases.get(f"eo_minus_{p['name']}", {})
            probes.append({
                **p,
        ...
        windows.append({**w, 'probes': probes})
    fam = {
    ...
    with (fam_dir / 'SUMMARY.txt').open('w', encoding='utf-8') as f:
        ...
        for w in windows:
            fatal = [p['offset'] for p in w['probes'] if not p['pass']]
            neutral = [p['offset'] for p in w['probes'] if p['hash_eq_target']]
            semantic = [p['offset'] for p in w['probes'] if p['pass'] and not p['hash_eq_target']]
            f.write(f"window w{w['window_index']:02d}: {w['start']}..{w['end']} rule_count={w['rule_count']} fatal_offsets={fatal} neutral_offsets={neutral} semantic_offsets={semantic}\n")
            for p in w['probes']:
                f.write(f"  {p['name']}: off={p['offset']} val=0x{p['value']:02x} pass={p['pass']} hash={p['hash']} hash_eq_target={p['hash_eq_target']} error={p['error']}\n")
...
with (out_dir / 'SUMMARY.txt').open('w', encoding='utf-8') as f:
    ...
    for fam in root['families']:
        ...
        for w in fam['windows']:
            ...
            neutral = [p['offset'] for p in w['probes'] if p['hash_eq_target']]
            semantic = [p['offset'] for p in w['probes'] if p['pass'] and not p['hash_eq_target']]
            f.write(f"  {w['start']}..{w['end']} fatal={fatal} neutral={neutral} semantic={semantic}\n")
