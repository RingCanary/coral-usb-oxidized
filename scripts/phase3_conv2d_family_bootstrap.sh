...
RUN_ID="phase3-conv2d-family-bootstrap-$(date -u +%Y%m%dT%H%M%SZ)"
...
printf "case_id\theight\twidth\tin_channels\tout_channels\teo_instr\tpc_instr\tparam_bytes\tinput_bytes\toutput_bytes\n" > "$OUT_DIR/size_table.tsv"

for spec in "${default_cases[@]}"; do
...
families = {}
for row in rows:
    key = f"eo{row['eo_instr']}_pc{row['pc_instr']}_param{row['param_bytes']}"
