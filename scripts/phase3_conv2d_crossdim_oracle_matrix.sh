...
RUN_ID="phase3-conv2d-crossdim-oracle-matrix-$(date -u +%Y%m%dT%H%M%SZ)"
...
for spec in "${pairs[@]}"; do
  ...
  "$REPO_ROOT/tools/conv_template_pipeline.sh" --out-dir "$PAIR_DIR/anchor" --height "$AH" --width "$AW" --in-channels "$CH" --out-channels "$OC" --kernel-size 1 --stride 1 --padding same --init-mode random_uniform >/dev/null
  "$REPO_ROOT/tools/conv_template_pipeline.sh" --out-dir "$PAIR_DIR/target" --height "$TH" --width "$TW" --in-channels "$CH" --out-channels "$OC" --kernel-size 1 --stride 1 --padding same --init-mode random_uniform >/dev/null
  ...
  cargo run --quiet --bin model_param_stream_dump -- --model "$PAIR_DIR/anchor/$(basename "$PAIR_DIR")_quant_edgetpu.tflite" --out "$PAIR_DIR/anchor_param_stream.bin" >/dev/null 2>&1 || true
  cargo run --quiet --bin model_param_stream_dump -- --model "$PAIR_DIR/target/$(basename "$PAIR_DIR")_quant_edgetpu.tflite" --out "$PAIR_DIR/target_param_stream.bin" >/dev/null 2>&1 || true
  # Above basename trick is not reliable; select actual *_edgetpu.tflite below.
...
for pair_dir in sorted(p for p in out_dir.iterdir() if p.is_dir()):
    ...
    target_hash = None
    for name in ['target_baseline','anchor_baseline','anchor_pc_oracle','anchor_eo_oracle','anchor_eopc_oracle']:
