#!/usr/bin/env python3
"""Recover Conv2D parameter layout behavior via single-hot compiled probes."""
...
def _parse_coord(text: str) -> Tuple[int, int, int, int]:
    parts = text.split(",")
    if len(parts) != 4:
        raise argparse.ArgumentTypeError(
            f"invalid coord '{text}' (expected y,x,in_channel,out_channel)"
        )
    try:
        y = int(parts[0])
        x = int(parts[1])
        in_channel = int(parts[2])
        out_channel = int(parts[3])
    except ValueError as exc:
        raise argparse.ArgumentTypeError(f"invalid coord '{text}': {exc}") from exc
    return y, x, in_channel, out_channel


def _coord_tag(y: int, x: int, in_channel: int, out_channel: int) -> str:
    return f"y{y:02d}_x{x:02d}_ic{in_channel:03d}_oc{out_channel:03d}"
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Run single-hot Conv2D probe matrix and recover parameter byte offset behavior.",
    )
    p.add_argument("--out-dir", help="Output directory. Default: traces/conv-layout-probe-<utc>.")
    ...
    p.add_argument("--in-channels", type=int, default=64)
    ...
    p.add_argument(
        ...
        type=_parse_coord,
        default=(0, 0, 0, 0),
        help="Reference probe y,x,in_channel,out_channel.",
    )
    p.add_argument(
        "--probe",
        action="append",
        type=_parse_coord,
        default=[],
        help="Probe y,x,in_channel,out_channel (repeatable).",
    )
    p.add_argument("--max-preview", type=int, default=32)
    return p
...
def main() -> int:
    ...
    if not args.probe:
        tail_ic = max(args.in_channels - 1, 0)
        tail_oc = max(args.out_channels - 1, 0)
        args.probe = [
            (0, 0, 0, 1 if args.out_channels > 1 else 0),
            (0, 0, 1 if args.in_channels > 1 else 0, 0),
            (0, 0, 1 if args.in_channels > 1 else 0, 1 if args.out_channels > 1 else 0),
            (0, 0, tail_ic // 2, tail_oc // 2),
            (0, 0, tail_ic, tail_oc),
        ]

    if args.reference not in args.probe:
        probes: List[Tuple[int, int, int, int]] = [args.reference] + args.probe
    else:
    ...
    for y, x, in_channel, out_channel in probes:
        tag = _coord_tag(y, x, in_channel, out_channel)
        quant_model = probe_root / f"conv_{tag}_quant.tflite"
        quant_meta = probe_root / f"conv_{tag}_quant.metadata.json"
        compile_log = probe_root / f"conv_{tag}_compile.log"
        ...
        gen_cmd = [
            ...
            str(repo_root / "tools" / "generate_conv2d_quant_tflite.py"),
            "--output",
            str(quant_model),
            "--metadata-out",
            str(quant_meta),
        ...
        compiled_model = probe_root / f"conv_{tag}_quant_edgetpu.tflite"
        ...
        blob_path = blob_root / f"conv_{tag}.params.bin"
    ...
    ref = next(
        (
            r
            for r in records
            if (r["y"], r["x"], r["in_channel"], r["out_channel"]) == args.reference
        ),
        None,
    )
    if ref is None:
    ...
    for rec in records:
        ...
        rec["diff_vs_reference"] = {
            "reference": _coord_tag(*args.reference),
    ...
    report = {
        ...
        "config": {
            ...
            "reference": {
                "y": args.reference[0],
                "x": args.reference[1],
                "in_channel": args.reference[2],
                "out_channel": args.reference[3],
            },
        },
        "records": records,
    }
    ...
    lines.append(
        f"reference={_coord_tag(*args.reference)} python={args.python_version} tf={args.tf_version} numpy={args.numpy_version}"
    )
    lines.append("")
    for rec in records:
        lines.append(
            f"[probe {rec['tag']}] y={rec['y']} x={rec['x']} ic={rec['in_channel']} oc={rec['out_channel']}"
        )
        lines.append(
