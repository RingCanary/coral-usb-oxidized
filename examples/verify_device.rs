...
fn verify_coral_device() -> (bool, String) {
    ...
    match device_result {
        Ok(device) => {
            ...
            if let Some(name) = device.name() {
                if name != EXPECTED_DEVICE_NAME {
                    return (false, format!("Device name mismatch: expected '{}', got '{}'", 
        ...
        Err(CoralError::DeviceNotFound) => {
            (false, "No Coral USB device found".to_string())
        },
        Err(CoralError::InitializationFailed) => {
            (false, "Device found but Edge TPU initialization failed - possible fake device".to_string())
        },
        Err(CoralError::PermissionDenied) => {
            (false, "Permission denied - check USB access rights".to_string())
        },
        Err(e) => {
...
fn main() {
    ...
    loop {
        ...
        if current_state != last_state {
            if current_state {
                println!("✅ Device CONNECTED at {}", chrono::Local::now().format("%H:%M:%S"));
                println!("{}", current_details);
            } else {
                println!("❌ Device DISCONNECTED at {}", chrono::Local::now().format("%H:%M:%S"));
